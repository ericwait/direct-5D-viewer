...
void MexViewSegmentation::execute(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) const
{
...
std::string MexViewSegmentation::check(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) const
{
	if (nrhs != 1)
		return "Not the right arguments for viewSegmentation!";

	return "";
}

void MexViewSegmentation::usage(std::vector<std::string>& outArgs, std::vector<std::string>& inArgs) const
{
	//inArgs.push_back("arrowFaces");
}

void MexViewSegmentation::help(std::vector<std::string>& helpLines) const
{
